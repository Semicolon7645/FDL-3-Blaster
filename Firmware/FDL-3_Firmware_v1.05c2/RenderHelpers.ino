...
void renderInfoMenu(){
  ...
  uView.print(versionNumberExtension);
  uView.display();
  firstMenuRun = false;
  uView.setFontType(0);
}

void renderOverviewMenu(){
  uView.print("Speed:");
  uView.print(currBlSettings.speedValue);
  uView.setCursor(0,26);
  uView.print("ROF:");
  uView.print(currBlSettings.rofValue);
  uView.setCursor(0,38);
  uView.print("Burst:");
  uView.print(burstMenu[currBlSettings.burstCount]);
  uView.display();
  firstMenuRun = false;
  uView.setFontType(0);
}
...
void renderAmmoCount()
{
  if(!liveKnobScrollMode)
  {
    uView.setCursor(0,0);
    uView.print("   ");
    if(currBlSettings.ammoCountStart != 0)
    {
      uView.setCursor(0,0);
      uView.print(currentAmmoCount);
    }
    uView.setCursor(0,14);
    uView.display();
    firstMenuRun = false;
